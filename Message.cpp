...
Message make_send(uint32_t source_id, uint32_t dest_id, uint32_t message_length ,const char *message, uint32_t trailing_msg) {
  Message send_message(source_id, dest_id, SEND, trailing_msg);
  memcpy(send_message.payload, &message_length, sizeof(message_length));
  strncpy(send_message.payload + fourBytes, message + (trailing_msg * size_of_data), message_length);
  return send_message;
}
